...
if __name__ == '__main__':
    ...
    for dist in [ 'e', 's', 'k', 'b']:
        if dist == 'e':
            ...
            folderpath = 'figs_som/euclidean.png'
        elif dist == 's':
            print 'computing spearman clustering'
            clusterid , _ = pycl.somcluster(prices, nxgrid=3, nygrid=3, dist=dist)
            folderpath = 'figs_som/spearman.png'
        elif dist=='k':
            print 'computing kendall clustering'
            clusterid , _ = pycl.somcluster(prices, nxgrid=3, nygrid=3, dist=dist)
            folderpath = 'figs_som/kendall.png'
        elif dist=='b':
            print 'computing city block clustering'
            clusterid, _ = pycl.somcluster( prices, nxgrid=3, nygrid=3, dist=dist)
            folderpath = 'figs_som/cityblock.png'
        cid = clusterid[:,0]*10 + clusterid[:,1]
        plot_eachclass(prices, cid, folderpath=folderpath)
